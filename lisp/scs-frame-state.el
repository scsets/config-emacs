...
;;; Commentary:
;;
;; Minimal frame geometry persistence.  Each saved state contains only:
;;
;;   left top pixel-width pixel-height fullscreen
;;
;; Use `scs/frame-state-save' before quitting Emacs to save the selected
;; frame under a label such as "1440x900+0+25".  After restart, use
;; `scs/frame-state-restore' and choose a saved label with completion.
...
(defconst scs/frame-state-keys
  '(left top pixel-width pixel-height fullscreen)
  "Alist keys written by `scs/frame-state-capture'.")

(defcustom scs/frame-state-default-label "default"
  "Fallback label for saved frame geometry entries."
  :group 'scs-frame-state
  :type 'string)
...
(defun scs/frame-state--validate (state)
  "Signal `user-error' unless STATE has the expected geometry keys."
...
(defun scs/frame-state--state-p (object)
  "Return non-nil when OBJECT looks like one frame geometry state."
  (and (listp object)
...
(defun scs/frame-state--entry-p (entry)
  "Return non-nil when ENTRY is a labeled frame geometry entry."
  (and (consp entry)
       (stringp (car entry))
       (scs/frame-state--state-p (cdr entry))))

(defun scs/frame-state--read-file (file)
  "Return the Lisp object stored in FILE, or nil when FILE is absent."
...
(defun scs/frame-state--write-all (entries file)
  "Write labeled frame geometry ENTRIES to FILE."
...
ENTRIES provides existing labels for completion.  DEFAULT is used as the
initial generated label."
...
also prompt for FILE."
  ...
  (let* ((file (or file scs/frame-state-file))
    ...
    (let* ((entries (cl-remove label (scs/frame-state-load-all file)
                               :key #'car :test #'string=))
