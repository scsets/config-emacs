...
\textbf{\textbf{Toggle:}} \texttt{M-x scs/org-line-prefixes-mode} / \texttt{M-x scs/org-line-prefixes-status}
...
\texttt{scs/org-regenerate-stationery} & \texttt{M-x} — rebuild without exporting\\
\end{tabular}
\end{center}
...
Wrong stationery in PDF & Stale cache & Touch source PDF or \texttt{M-x scs/org-regenerate-stationery}\\
...
\texttt{M-x scs/org-line-prefixes-status} & Check \texttt{!} / => hook\\
\texttt{M-x scs/org-regenerate-stationery} & Rebuild faded letterhead from buffer keywords\\
\end{tabular}
\end{center}
\section*{File index}
